...
init python:
    ...
    class Game2048:
        ...
        def move(self, direction):
            ...
            for i in range(4):
                new_row = [num for num in self.grid[i] if num != 0]
...
screen game_2048():
    ...
    frame:
        ...
        vbox:
            spacing 10
            label "2048" xalign 0.5
            hbox:
                spacing 10
                text "Очки: [game.score]"
                button:
                    text "Рестарт"
                    action Function(game.__init__)
                button:
                    text "Выход"
                    action Jump("Post_2048")
            ...
            if game.is_game_over():
                text "Игра окончена!" xalign 0.5

    key "K_LEFT" action Function(game.move, 0)
    key "K_DOWN" action Function(game.move, 1)
    key "K_RIGHT" action Function(game.move, 2)
    key "K_UP" action Function(game.move, 3)
...
init python:
    ...
    # Фигуры тетрамино (цвета в формате HEX)
    SHAPES = [
        [[1, 1, 1, 1]], # I
        [[1, 1], [1, 1]], # O
        [[1, 1, 1], [0, 1, 0]], # T
    ...
    class Tetris:
        ...
        def clear_lines(self):
            ...
            if lines_cleared > 0 and self.score // 2000 > self.level - 1:
                self.level += 1
...
screen tetris():
    add "ZP.png" pos (400,125)
    default game = Tetris()
    timer 0.5 action Function(game.update) repeat True
    $ actio=Jump("Post_tetris_1") if day==1 else Jump("Post_tetris_2")
    imagebutton:
        idle "НЕТ.png"
        pos (1000,126)
        action actio
    frame:
        ...
        for y in range(GRID_HEIGHT):
            for x in range(GRID_WIDTH):
                if game.grid[y][x]:
                    add Solid(game.grid[y][x], xsize=CELL_SIZE-1, ysize=CELL_SIZE-1, xpos=x*CELL_SIZE, ypos=y*CELL_SIZE)
        ...
        if not game.game_over:
            for y, row in enumerate(game.current_piece["shape"]):
                for x, cell in enumerate(row):
                    if cell:
                        add Solid(game.current_piece["color"], xsize=CELL_SIZE-1, ysize=CELL_SIZE-1, 
                                xpos=(game.current_piece["x"]+x)*CELL_SIZE, 
                                ypos=(game.current_piece["y"]+y)*CELL_SIZE)

        # Интерфейс
        vbox:
            xpos GRID_WIDTH * CELL_SIZE + 10 - 10
            ypos 200
            spacing 10
            text "Очки: [game.score]" color "#FFF"
            text "Уровень: [game.level]" color "#FFF"
            if game.game_over:
                text "Конец!" color "#F00" size 30
            if game.paused:
                text "Пауза" color "#FF0" size 30

    # Управление
    python:
        ae=Function(game.current_piece.__setitem__, "x", game.current_piece["x"] - 1) if game.valid_move(game.current_piece, -1, 0) and not game.game_over else NullAction()
        be=Function(game.current_piece.__setitem__, "x", game.current_piece["x"] + 1) if game.valid_move(game.current_piece, 1, 0) and not game.game_over else NullAction()
        ce=Function(game.current_piece.__setitem__, "y", game.current_piece["y"] + 1) if game.valid_move(game.current_piece, 0, 1) and not game.game_over else NullAction()
        de=Function(game.rotate_piece) if not game.game_over else NullAction()
    key "K_LEFT" action ae
    key "K_RIGHT" action be
    key "K_DOWN" action ce
    key "K_UP" action de
    key "K_SPACE" action ToggleDict(game, "paused")
    key "K_r" action Function(game.__init__)
init python:
    import random
    ...
    class SnakeGame:
        ...
        def reset(self):
            ...
            self.score = 0
            self.game_over = False
            self.speed = 0.15  # Интервал между движениями (секунды)
...
screen snake():
    imagebutton:
        ...
        action Jump("Post_snake")
    default game = SnakeGame()
    timer game.speed action Function(game.update) repeat True
    ...
    frame:
        ...
        for x in range(GRID_WIDTH):
            for y in range(GRID_HEIGHT):
                add Solid("#222", xsize=CELL_SIZE-1, ysize=CELL_SIZE-1, xpos=x*CELL_SIZE, ypos=y*CELL_SIZE)

        # Еда
        add Solid("#FF5555", xsize=CELL_SIZE-1, ysize=CELL_SIZE-1, 
                 xpos=game.food[0]*CELL_SIZE, ypos=game.food[1]*CELL_SIZE)

        # Змея
        for segment in game.snake:
            add Solid("#44FF44", xsize=CELL_SIZE-1, ysize=CELL_SIZE-1, 
                     xpos=segment[0]*CELL_SIZE, ypos=segment[1]*CELL_SIZE)

        # Интерфейс
        vbox:
            xalign 0.5
            ypos 50
            text "Очки: [game.score]" color "#FF0" size 24
            if game.game_over:
                text "R - рестарт" color "#F00" size 30 xalign 0.5

    # Управление
    python:
        a=Function(setattr, game, "direction", (-1, 0)) if game.direction != (1, 0) else NullAction
        b=Function(setattr, game, "direction", (1, 0)) if game.direction != (-1, 0) else NullAction
        c=Function(setattr, game, "direction", (0, -1)) if game.direction != (0, 1) else NullAction
        d=Function(setattr, game, "direction", (0, 1)) if game.direction != (0, -1) else NullAction
    key "K_LEFT" action a
    key "K_RIGHT" action b
    key "K_UP" action c
    key "K_DOWN" action d

    key "K_r" action Function(game.reset)
    
screen Computer:
    frame:
        background None
        xysize (1920, 1080)
        key "dismiss" action Hide("LaxLux")
    add "Phon.png":
        yzoom 0.14
        xzoom 0.17
        # (200,200)
        pos (396,205)
    $ actio = Play("sound", "pip.mp3") if day!=2 or laxlux else Show("Poisk")
    imagebutton:
        pos (430,230)
        idle "Brauser.png"
    ...
    if zadacha[0]:
     imagebutton:
        ...
        action (Hide("Computer"),Hide("Pismo"),Hide("V"),Hide("LaxLux"))
...
screen V:
    ...
    button:
        ...
        xsize 185
        ysize 40
        action Function(renpy.call_in_new_context, "proverka")
        #background None
        #hover_background None
    button:
        xpos 530
...
screen Skachivaem:
    ...
    add "Skachivaem.png":
        align (0.5,0.4)
    $ tex=str(secund)+"%"
    text tex align (0.5,0.45) size 50 color "#000000"
...
screen Proverka:
    ...
    python:
     if secund<100:
      ...
      tex="Обнаружен вирус. Удаляем вирус."
     else:
      tex="Всё чисто."
     siz=50 if "%" in tex else 15
    text tex align (0.5,0.45) size siz color "#000000"
screen dating_screen(current=1):
    add "g[{}].png".format(current) at Transform(zoom=0.4) align (0.5,0.5)
    hbox:
            spacing 200
            align (0.5, 0.7)
